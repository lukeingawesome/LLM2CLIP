...
LABEL maintainer=kolucas1219@gmail.com
ENV LANG=C.UTF-8
ENV LC_ALL=C.UTF-8
...
RUN --mount=type=bind,from=build-pillow,source=/tmp/dist,target=/tmp/dist \
    ...
    python -m pip install -r /tmp/train/requirements.txt
...
ARG CONDA_ENV_FILE2=/tmp/train/requirements.txt
COPY --link ../reqs/train-environment.yaml ${CONDA_ENV_FILE}
COPY --link ../reqs/requirements.txt ${CONDA_ENV_FILE2}
